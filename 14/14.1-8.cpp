...
public:
    [[nodiscard]] Node* Search(const T& key) const {
    ...
    [[nodiscard]] size_t Rank(const Node* x) const {
        ...
        size_t r = (x->left ? x->left->size : 0) + 1;
        auto y = x;
        while (y != root.get()) {
            if (y == y->parent->right.get()) {
                r += 1 + (y->parent->left ? y->parent->left->size : 0);
            }
            y = y->parent;
        }
        return r;
...
size_t numOfPairsIntersecting(std::vector<std::pair<double, double>>& chords) {
    std::vector<std::tuple<double, double, bool>> endpoints;
    ...
    for (const auto& [p, q, left] : endpoints) {
        if (left) {
            ...
            auto node = tree.Search(q);
            assert(node);
            tree.Delete(q);
            size_t sz = tree.root ? tree.root->size : 0;
            count += sz;
        }
    }
...
int main() {
    ...
    std::vector<std::pair<double, double>> chords;
    std::uniform_real_distribution<> dist(0.0, 1.0);
    for (size_t i = 0; i < N; i++) {
        chords.emplace_back(dist(gen), dist(gen));
    }

    std::cout << numOfPairsIntersecting(chords) << '\n';

    chords.clear();
    for (size_t i = 0; i < N; i++) {
        chords.emplace_back(i * 1.0 / (2 * N), 0.5 + i * 1.0 / (2 * N));
    }
