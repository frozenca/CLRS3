...
public:
    ...
    [[nodiscard]] const Node* MinIntervalSearch(double begin, double end) const {
        assert(begin <= end);
        return MinIntervalSearch(root.get(), begin, end);
    }

private:
    [[nodiscard]] const Node* MinIntervalSearch(const Node* x, double begin, double end) const {
        if (!x) {
            return x;
        }
        if (x->left && x->left->maximum >= begin) {
            const auto y = MinIntervalSearch(x->left.get(), begin, end);
            if (y) {
                return y;
            } else if (begin <= x->end && end >= x->begin) {
                return x;
            } else {
                return nullptr;
            }
            ...
            return MinIntervalSearch(x->right.get(), begin, end);
        }
    }
