...
public:
    ...
    [[nodiscard]] size_t Rank(const T& k) const {
        return Rank(root.get(), k);
    }

private:
    [[nodiscard]] size_t Rank(Node* curr, const T& k) const {
        if (!curr) {
            return 0;
        }
        if (k == curr->key) {
            return (curr->left ? curr->left->size : 0) + 1;
        } else if (k < curr->key) {
            return Rank(curr->left, k);
        } else {
            return (curr->left ? curr->left->size : 0) + 1 + Rank(curr->right, k);
        }
    }
