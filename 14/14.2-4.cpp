...
public:
    ...
    [[nodiscard]] std::vector<T> Enumerate(const T& a, const T& b) const {
        assert(a <= b);
        Node* begin = Search(root.get(), a);
        std::vector<T> v;
        if (begin && begin->key < a) {
            begin = Successor(begin);
        }
        while (begin && begin->key <= b) {
            v.push_back(begin->key);
            begin = Successor(begin);
        }
        return v;
    }
