...
class TreeArray {
    ...
    std::array<std::optional<T>, (1u << (2 * k))> optional_data = {0};
    std::array<std::size_t, (1u << k)> summary = {0};

public:
    void Insert(std::size_t value, const T& dat) {
        data[value]++;
        optional_data[value] = dat;
        summary[value >> k] == 1;
    }
    ...
    [[nodiscard]] std::optional<size_t> Successor(std::size_t value) const {
        ...
        auto first_search = sr::find(data | srv::drop(value + 1) | srv::take((index + 1) >> k), 1);
        if (first_search != cluster_end) {
            ...
            if (second_search != std::end(summary)) {
                ...
                return std::distance(std::begin(data), sr::find(data | srv::drop(dist >> k) | srv::take((dist + 1) >> k), 1));
            } else {
                return {};
            }
        }
