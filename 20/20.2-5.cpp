...
class ProtoVEB {
    ...
    std::unique_ptr<ProtoVEB<t - 1, bool>> summary;
    std::array<std::unique_ptr<ProtoVEB<t - 1, T>>, (1u << (1u << (t - 1)))> clusters;

    static constexpr std::size_t v = (1u << (1u << (t - 1)));

public:
    ProtoVEB() : summary {std::make_unique<ProtoVEB<t - 1, bool>>()} {
        for (std::size_t i = 0; i < clusters.size(); i++) {
            clusters[i] = std::make_unique<ProtoVEB<t - 1, T>>();
        }
    }
    ...
    void Insert(std::size_t x, const T& value = T{}) {
        clusters[high(x)]->Insert(low(x), value);
        summary->Insert(high(x), true);
    }

    void Delete(std::size_t x) {
        clusters[high(x)]->Delete(low(x));
        if (!sr::any_of(clusters[high(x)], [](auto& n){return n > 0;})) {
...
template <typename T>
class ProtoVEB<0, T> {
    std::array<std::optional<T>, 2> data = {false};
...
public:
    ...
    void Insert(std::size_t x, const T& value) {
        assert(x < 2);
        data[x] = value;
    }
