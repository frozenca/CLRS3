...
class WeightedVerticeGraph {
    const std::size_t n;
    std::vector<double> w;
    std::vector<std::list<std::size_t>> adj;
    std::unordered_map<std::size_t, std::list<std::size_t>::iterator> edges;
public:
    WeightedVerticeGraph(std::size_t n, const std::vector<double>& w) : n {n}, w (w), adj(n) {
        assert(w.size() == n && sr::all_of(w, [](auto weight){return weight > 0.0;}));
    }
    ...
    [[nodiscard]] std::vector<std::size_t> ApproxMinWeightVC() {
        std::size_t E = 0;
...
int main() {
    ...
    std::uniform_int_distribution<> dist_idx(0, 29);
    std::vector<double> w;
    w.reserve(n);
    for (std::size_t i = 0; i < n; i++) {
        w.push_back(dist(gen));
    }
    WeightedVerticeGraph g(n, w);
    for (std::size_t i = 0; i < n * 2; i++) {
        g.addEdge(dist_idx(gen), dist_idx(gen));
    }
    auto AMWVC = g.ApproxMinWeightVC();
    for (auto idx : AMWVC) {
        std::cout << idx << ' ';
    }
    std::cout << '\n';
