...
std::vector<std::unordered_set<std::size_t>> GreedySetCover(const std::unordered_set<std::size_t>& X,
    ...
    while (!U.empty()) {
        ...
        for (std::size_t i = 0; i < F.size(); i++) {
            auto curr_inter_size = getIntersectionSize(U, F[i]);
...
int main() {
    ...
    auto covers = GreedySetCover(X, F);
