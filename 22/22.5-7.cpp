...
public:
    ...
    std::vector<std::size_t> getTopologicalSort() {
        infos = std::vector<info_t>(n, std::make_tuple(Color::White, 0, 0, -1));
        std::size_t time = 0;
        std::stack<std::size_t> S;
    ...
    [[nodiscard]] bool isSemiconnected() {
        auto cg = getComponentGraph();
        auto tl = cg.getTopologicalSort();
        for (std::size_t i = 0; i < tl.size(); i++) {
            if (sr::find(cg.adj[tl[i]], tl[i + 1]) == cg.adj[tl[i]].end()) {
                return false;
            }
        }
        if (!tl.empty()) {
            if (sr::find(cg.adj[tl.back()], tl.front()) == cg.adj[tl.back()].end()) {
                return false;
            }
        }
        return true;
