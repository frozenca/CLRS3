...
public:
    ...
    bool Augment(const std::vector<std::size_t>& path,
                 std::size_t src, std::size_t dst, std::size_t criteria) {
    ...
    void MaxFlowByScaling() {
        ...
        std::size_t K = std::pow(2, std::floor(std::log2(max_capacity)));
        while (K) {
            while (true) {
                ...
                if (!Augment(argPathPrev, source, sink, K)) {
                    break;
                }
            }
            K /= 2;
