...
class FlowNetwork final {
    ...
    // (src)
    std::vector<std::list<std::size_t>> backward_adj;
...
public:
    FlowNetwork(std::size_t source = 0, std::size_t sink = n - 1) : adj(n), res(n), backward_adj(n),
                                                                    undirected_adj(n), source (source), sink (sink) {}

    FlowNetwork(const FlowNetwork& g) : adj(n), res(n), backward_adj(n) {
    ...
    void addEdge(std::size_t src, std::size_t dst, std::size_t capacity) {
        ...
        if (!edges.contains(src * n + dst) && !edges.contains(dst * n + src)) {
            ...
            backward_adj[dst].emplace_front(src);
    ...
    std::size_t GenericPushRelabel() {
        ...
        while (!q.empty()) {
            auto curr = q.front();
            q.pop();
            for (auto prev : backward_adj[curr]) {
                if (prev != source && Relabelable(prev, excess, height)) {
                    Relabel(prev, height);
                    q.push(prev);
                }
            }
