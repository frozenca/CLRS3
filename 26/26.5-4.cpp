...
public:
    ...
    bool Discharge(std::size_t v, std::vector<int>& excess, std::vector<std::size_t>& height,
                   std::vector<std::list<std::size_t>::iterator>& current_edges,
                   std::vector<std::list<std::size_t>>& height_list,
                   std::size_t& highest_height) {
        bool to_reset = false;
        while (excess[v] > 0) {
            if (current_edges[v] == undirected_adj[v].end()) {
                assert(Relabelable(v, excess, height));
                Relabel(v, height);
                height_list[height[v]].push_back(v);
                highest_height = height[v];
                current_edges[v] = undirected_adj[v].begin();
            } else if (Pushable(v, *current_edges[v], excess, height)) {
                auto w = *current_edges[v];
                Push(v, w, excess);
                if (excess[w] > 0 && w != sink && w != source) {
                    height_list[height[w]].push_back(w);
                    if (height[w] > highest_height) {
                        highest_height = height[w];
                    } else if (height_list[highest_height].empty()) {
                        to_reset = true;
                    }
                }
            } else {
    ...
    std::size_t RelabelToFront() {
        ...
        std::vector<std::list<std::size_t>> height_list (2 * n);
        std::size_t highest_height = 0;
        for (std::size_t u = 0; u < n; u++) {
            if (u != source && u != sink && excess[u] > 0) {
                height_list[height[u]].push_back(u);
            }
        }
        while (!height_list[highest_height].empty()) {
            auto u = height_list[highest_height].front();
            height_list[highest_height].pop_front();
            bool to_reset = Discharge(u, excess, height,
                      current_edges, height_list,
                      highest_height);
            if (to_reset || height_list[highest_height].empty()) {
                for (std::size_t v = height_list.size() - 1; v < height_list.size(); v--) {
                    if (!height_list[v].empty()) {
                        highest_height = v;
                        break;
                    }
                }
