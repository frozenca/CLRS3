...
public:
    ...
    void Discharge(std::size_t v, std::vector<int>& excess, std::vector<std::size_t>& height,
                   std::vector<std::list<std::size_t>::iterator>& current_edges,
                   std::queue<std::size_t>& Q, std::unordered_set<std::size_t>& Q_unique) {
        while (excess[v] > 0) {
            if (current_edges[v] == undirected_adj[v].end()) {
                ...
                Push(v, w, excess);
                if (excess[w] > 0 && w != source && w != sink) {
                    if (!Q_unique.contains(w)) {
                        Q.push(w);
                        Q_unique.insert(w);
                    }
                }
    ...
    std::size_t RelabelToFront() {
        ...
        std::queue<std::size_t> Q;
        std::unordered_set<std::size_t> Q_unique;
        ...
        while (!Q.empty()) {
            auto u = Q.front();
            Discharge(u, excess, height, current_edges, Q, Q_unique);
            Q.pop();
            Q_unique.erase(u);
        }
