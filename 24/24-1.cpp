...
public:
    ...
    void Relax(std::size_t u, std::size_t v, bool order) {
        T w = getWeight(u, v);
        if ((!order && u < v || order && u > v) && pinfos[v].d > pinfos[u].d + w) {
    ...
    bool BellmanFord(std::size_t s) {
        ...
        for (std::size_t i = 0; i < std::ceil(n / 2); i++) {
            for (std::size_t u = 0; u < n; u++) {
                for (const auto& [v, w] : adj[u]) {
                    Relax(u, v, false);
                }
            }
            for (std::size_t u = n - 1; u < n; u--) {
                for (const auto& [v, w] : adj[u]) {
                    Relax(u, v, true);
                }
            }
