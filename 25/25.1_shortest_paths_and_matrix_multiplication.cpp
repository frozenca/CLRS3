...
public:
    ...
    [[nodiscard]] std::array<T, n * n> ExtendShortestPaths(const std::array<T, n * n>& L) const {
        std::array<T, n * n> L_;
        sr::fill(L_, std::numeric_limits<T>::max());
        for (std::size_t i = 0; i < n; i++) {
            for (std::size_t j = 0; j < n; j++) {
                for (std::size_t k = 0; k < n; k++) {
                    L_[i * n + j] = std::min(L_[i * n + j], L[i * n + k] + W[k * n + j]);
                }
            }
        }
        return L_;
    ...
    [[nodiscard]] std::array<T, n * n> SlowAllPairsShortestPaths() const {
        auto L = W;
...
int main() {
    ...
    auto L1 = g.SlowAllPairsShortestPaths();
