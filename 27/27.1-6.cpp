...
template <Scalar T1, Scalar T2, Scalar T3 = std::common_type_t<T1, T2>>
T3 MatSubLoop(const Matrix<T1>& m1, const Matrix<T2>& m2, T3& val, std::size_t i, std::size_t j, std::size_t j2) {
    if (j == j2) {
        val += m1(i, j) * m2(j, 0);
    } else {
        std::size_t mid = (j + j2) / 2;
        std::jthread t(&MatSubLoop<T1, T2, T3>, std::cref(m1), std::cref(m2), std::ref(val), i, j, mid);
        MatSubLoop(m1, m2, val, i, mid + 1, j2);
    }
    return val;
}

template <Scalar T1, Scalar T2, Scalar T3 = std::common_type_t<T1, T2>>
Matrix<T3> FastMatVec(const Matrix<T1>& m1, const Matrix<T2>& m2) {
    std::size_t M = m1.R;
    std::size_t K = m1.C;
    std::size_t N = m2.C;
    assert(m2.R == K && N == 1);
    Matrix<T3> m3 (M, N);
    std::fill(se::unseq, m3.begin(), m3.end(), 0);
    std::vector<std::size_t> v (M);
    std::iota(v.begin(), v.end(), 0);
    std::transform(se::unseq, m3.begin(), m3.end(),
                   v.begin(), m3.begin(),
                   [&](auto& val, auto idx){return MatSubLoop(m1, m2, val, idx, 0, M - 1);});
    return m3;
}
