...
template <Scalar T>
Matrix<T> P_LUPSolve(const Matrix<T>& L, const Matrix<T>& U, const Matrix<std::size_t>& P,
                   const Matrix<T>& b) {
    std::size_t n = L.R;
    assert(L.C == n && U.R == n && U.C == n && P.R == 1 && P.C == n && b.R == 1 && b.C == n);
    Matrix<T> x (1, n);
    ...
    for (std::size_t i = 0; i < n; i++) {
        T partial_sum = 0;
        std::for_each(se::unseq, idx.begin(), idx.begin() + i, [&i, &partial_sum, &L, &y](auto j) {
            partial_sum += L(i, j) * y(0, j);
        });
        y(0, i) = b(0, P(0, i)) - partial_sum;
    }
    for (std::size_t i = n - 1; i < n; i--) {
        T partial_sum = 0;
        std::for_each(se::unseq, idx.begin() + i + 1, idx.end(), [&i, &partial_sum, &U, &x](auto j) {
            partial_sum += U(i, j) * x(0, j);
        });
        x(0, i) = (y(0, i) - partial_sum) / U(i, i);
    }
...
int main() {
    ...
    Matrix<double> L {{1.0, 0.0, 0.0},
                      {0.2, 1.0, 0.0},
                      {0.6, 0.5, 1.0}};
    Matrix<double> U {{5.0, 6.0, 3.0},
                      {0.0, 0.8, -0.6},
                      {0.6, 0.5, 2.5}};
    Matrix<std::size_t> P {{2, 0, 1}};
    Matrix<double> b {{3.0, 7.0, 8.0}};
