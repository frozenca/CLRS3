...
template <Scalar T>
Matrix<T> Transpose(const MatrixView<T>& A) {
    std::size_t n = A.R;
    assert(A.C == n);
    Matrix<T> AT (n, n);
    for (std::size_t i = 0; i < n; i++) {
...
std::vector<std::size_t> global_index_sequence (10'000);

template <Scalar T>
Matrix<T> PTranspose(const Matrix<T>& A) {
    std::size_t n = A.R;
    assert(A.C == n);
    Matrix<T> AT (n, n);
    std::for_each(se::unseq, global_index_sequence.begin(),
                  global_index_sequence.begin() + n * n,
                  [&A, &AT, &n](auto idx) {
                      auto i = idx / n;
                      auto j = idx % n;
                      AT(i, j) = A(j, i);
...
template <Scalar T>
Matrix<T> PTranspose(const MatrixView<T>& A) {
    std::size_t n = A.R;
    assert(A.C == n);
    Matrix<T> AT (n, n);
    std::for_each(se::unseq, global_index_sequence.begin(),
...
template <Scalar T>
Matrix<T> InvSPD(const MatrixView<T>& A) {
    std::size_t n = A.R;
    assert(A.C == n);
    // skip for verification of symmetric positive definite.
    std::size_t h = n / 2;
...
template <Scalar T>
Matrix<T> PInvSPD(const MatrixView<T>& A) {
    std::size_t n = A.R;
    assert(A.C == n);
    // skip for verification of symmetric positive definite.
    std::size_t h = n / 2;
    ...
    auto Binv = PInvSPD(B);
    auto W = C * Binv;
    auto WT = PTranspose(W);
    auto X = W * CT;
    auto S = D - X;
    auto Sinv = PInvSPD(S);
    V = Sinv;
    auto Y = Sinv * W;
    auto YT = PTranspose(Y);
    Q = YT * -1.0;
    U = Y * -1.0;
    auto Z = WT * Y;
    R = Binv + Z;
...
template <Scalar T>
Matrix<T> PInvSPD(const Matrix<T>& A) {
    std::size_t n = A.R;
    assert(A.C == n);
    // skip for verification of symmetric positive definite.
    std::size_t h = n / 2;
...
int main() {
    std::iota(global_index_sequence.begin(), global_index_sequence.end(), 0);

    Matrix<double> A {{2.0, 1.0, 0.0, 1.0},
                      {-1.0, 2.0, -1.0, 0.0},
                      {0.0, -1.0, 2.0, 1.0},
                      {1.0, 0.0, 1.0, 2.0}};
