...
template <Scalar T>
std::pair<Matrix<T>, Matrix<T>> P_LUDecomposition(Matrix<T>& A) {
    assert(A.R == A.C);
    std::size_t n = A.R;
    Matrix<T> L (n, n);
    Matrix<T> U (n, n);
    ...
    for (std::size_t k = 0; k < n; k++) {
        std::for_each(se::unseq, v.begin(), v.begin() + k, [&k, &L, &U](auto i){
            U(k, i) = 0;
            L(i, k) = 0;
        });
        U(k, k) = A(k, k);
        L(k, k) = 1;
        std::for_each(se::unseq, v.begin() + k + 1, v.end(), [&k, &L, &U, &A](auto i){
            L(i, k) = A(i, k) / U(k, k);
            U(k, i) = A(k, i);
        });
        std::for_each(se::unseq, v.begin() + k + 1, v.end(), [&k, &L, &U, &A, &v](auto i){
            std::for_each(se::unseq, v.begin() + k + 1, v.end(), [&k, &L, &U, &A, &v, &i](auto j) {
                A(i, j) -= L(i, k) * U(k, j);
            });
        });
    }
