...
void PMatrixMultiplyRecursiveRef(MatrixView<T>& C, const MatrixView<T>& A, const MatrixView<T>& B) {
    ...
    if (C.R == 1) {
        ...
        auto C22 = C.submatrix(H, H, N - 1, N - 1);
        {
            ...
            PMatrixMultiplyRecursiveRef<T>(C22, A21, B12);
        }
        {
            std::jthread t5(&PMatrixMultiplyRecursiveRef<T>, std::ref(C11), std::cref(A12), std::cref(B21));
            std::jthread t6(&PMatrixMultiplyRecursiveRef<T>, std::ref(C12), std::cref(A12), std::cref(B22));
            std::jthread t7(&PMatrixMultiplyRecursiveRef<T>, std::ref(C21), std::cref(A22), std::cref(B21));
            PMatrixMultiplyRecursiveRef<T>(C22, A22, B22);
        }
    }
}
...
template <Scalar T>
void PMatrixMultiplyLessMem(Matrix<T>& C, const Matrix<T>& A, const Matrix<T>& B) {
    std::fill(se::unseq, C.begin(), C.end(), 0);
    PMatrixMultiplyRecursive<T>(C, A, B);
}
...
int main() {
    ...
    Matrix<int> m4 (N, N);
    PMatrixMultiplyLessMem(m4, m1, m2);
