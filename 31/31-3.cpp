...
static const Matrix<std::size_t> fibo_base {{0, 1}, {1, 1}};
static const Matrix<std::size_t> I_2 {{1, 0}, {0, 1}};

template <Scalar T>
Matrix<T> MatPow(const Matrix<T>& A, std::size_t n, const Matrix<T>& I) {
    if (n & 1) {
        if (n == 1) {
            return A;
        } else {
            return A * MatPow(A * A, (n - 1) >> 1, I);
        }
    } else {
        if (n == 0) {
            return I;
        } else {
            return MatPow(A * A, n >> 1, I);
        }
    }
}

std::size_t Fibonacci(std::size_t k) {
    return MatPow(fibo_base, k + 1, I_2)(0, 0);
}
